  ...
  return (
    <section className="space-y-6 py-8">
      <div>
        ...
        <p className="text-slate-500 text-sm mt-1">A detailed psychological portrait of how you operate.</p>
      </div>
      ...
      {!hasContent ? (
        ...
        <div className="space-y-4">
          ...
          {SECTION_CONFIG.map(({ key, title, icon, bg, border, accent }) => {
            ...
            return (
              <div key={key} className={`p-5 rounded-2xl ${bg} border ${border}`}>
            ...
            <div className="p-5 rounded-2xl bg-white border border-slate-100">
              ...
              <div className="flex flex-wrap gap-2">
                {archetype.famous_examples.map(name => (
                  <span
                    key={name}
                    className="px-3 py-1.5 rounded-full bg-slate-50 border border-slate-200 text-slate-700 text-sm font-medium"
                  >
                    {name}
                  </span>
                ))}
